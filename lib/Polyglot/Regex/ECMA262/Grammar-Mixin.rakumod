...
token routine_declarator:sym<regex:ecma> {
    :my $*LINE_NO := {
...
token term:sym<regex_ecma_quote> {
    'rx:ecma' # MUST be first, to prioritize over simple rx// token
    ...
    [
    || <?{$*ECMA262-HAD-POSTOPTIONS}> <ECMA262-Regex-option>*       # shouldn't be necessary
    || '/' <ECMA262-Regex-option>*                                  # consume the slash
    || <.panic: "Unable to parse regex; couldn't find final '/'">   # wtf was this
    ]
}

token term:sym<regex_ecma_match> {
    'm:ecma' # MUST be first, to prioritize over simple rx// token
    :my $*ECMA262-ALLOW-POSTOPTIONS = True;
    :my $*ECMA262-HAD-POSTOPTIONS   = False;
    :my $*ECMA262-ENDER := '/';
